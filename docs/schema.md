...
| `transactions` | Append-only ledger. `kiosk_code`, `location_code`, `user`, `started_at`, `completed_at`, `status`. Optional `door_id` (text, indexed) is a per-door/terminal attribution tag — empty for single-kiosk installs; set by the RFID enclosure_diff flow (`cart.DoorID`) or supplied by a manual terminal on commit (`?door=` URL param). Pure attribution, never gates auth/correlation. Also carries `closed_by_admin` (FK, populated when the row was created by an admin force-close) and `command_id` (unique-when-non-empty, idempotency anchor for controller-forwarded closes). |
