...
| `POST` | `/api/kiosk/timeclock/punch` | none | Live punch — the worker's own (self) or a foreman punching a crew member when `target_user_code` differs. Body: `{user_code, direction: "in"\|"out", target_user_code?, acknowledge?}`. Backdating and admin-force are NOT accepted here; `acknowledge:true` is the worker/foreman "clock out anyway" past the open-tools block (recorded on the punch). Funnels through `timeclock.PerformPunch`; the foreman+same-group gate is re-enforced inside the txn. **409** `{error: "already_clocked_in"\|"not_clocked_in"\|"open_checkouts", …}` for expected conflicts. The `open_checkouts` body carries the blocking rows — **fleet-wide in managed mode**, each tagged with the `kiosk_code` to return it to. |
...
| `POST` | `/api/self/timeclock/punch` | worker | The authenticated worker's own live punch. Body: `{direction, acknowledge?}` — the target is always the session's worker (forced `source=self`; no backdate/foreman powers). `acknowledge:true` is the worker's "clock out anyway" past the fleet-wide open-tools block. 409 conflict bodies match the kiosk punch endpoint (the `open_checkouts` body lists tools out anywhere in the fleet, each with its building). |
