...
| `POST` | `/api/kiosk/items/{id}/adjust` | admin | Change `quantity_on_hand` + write a `stock_adjustments` audit row in one transaction. **400** for serialized items — their quantity is derived from the active instance count, so change it by adding/decommissioning/reactivating/deleting instances instead. |
| `POST` | `/api/kiosk/checkouts/by-line/{transaction_line_id}/close` | admin | Admin force-close one `open_checkouts` row. Body: `{reason: "lost"\|"returned_offline"\|"damaged"\|"other", notes?}`. Writes one `transactions` row + one `transaction_lines` row (`action="admin_close"`) and deletes the open row, all in one DB transaction. `lost` / `damaged` also drop the item's count: for quantity-tracked items by decrementing `quantity_on_hand` + writing a `stock_adjustments` row; for serialized items by flipping `item_instances.active=false` + writing an `instance_audit` decommission row (the derived `quantity_on_hand` follows the active count — no `stock_adjustments` row). |
