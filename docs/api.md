...
| `POST` | `/api/kiosk/timeclock/punch` | none | Live punch — the worker's own (self) or a foreman punching a crew member when `target_user_code` differs. Body: `{user_code, direction: "in"\|"out", target_user_code?, acknowledge?, job_code?}` (`job_code` is an optional job / work-order tag, meaningful on a clock-in). Backdating and admin-force are NOT accepted here; `acknowledge:true` is the worker/foreman "clock out anyway" past the open-tools block (recorded on the punch). Funnels through `timeclock.PerformPunch`; the foreman+same-group gate is re-enforced inside the txn. **409** `{error: "already_clocked_in"\|"not_clocked_in"\|"open_checkouts", …}` for expected conflicts. The `open_checkouts` body carries the blocking rows — **fleet-wide in managed mode**, each tagged with the `kiosk_code` to return it to. |
...
| `POST` | `/api/kiosk/timeclock/admin-punch` | admin | Manual/corrective punch: backdating (`occurred_at`, RFC3339) and `force` clock-out past the open-tools block are admin-only. Body: `{user_code, direction, reason, occurred_at?, force?, job_code?}` (reason required; `job_code` optional). |
...
| `GET` | `/api/kiosk/reports/timeclock.csv` | admin | Raw-punch CSV — the **payroll contract** (no rounding/overtime/pay-period logic). Includes a `job_code` column (appended last) for cost-by-job pivots. Query: `?from=&to=`, `?user_code=`. |
...
| `POST` | `/api/controller/kiosks/{code}/timeclock/punch` | admin | **Controller only.** Records a controller-admin punch AT the target kiosk (kiosks are the only punch writers) via the `timeclock.punch` command. Body: `{user_code, direction, reason, occurred_at?, force?, job_code?}`. Server-generates `command_id` for idempotent replay; 503 on offline. |
...
| `GET` | `/api/controller/reports/timeclock.csv` | admin | **Controller only.** Raw fleet punches as CSV (payroll contract), with `kiosk_code` so downstream can demux by site and a `job_code` column for cost-by-job pivots. |
...
| `POST` | `/api/self/timeclock/punch` | worker | The authenticated worker's own live punch. Body: `{direction, acknowledge?, job_code?}` (`job_code` optional, meaningful on a clock-in) — the target is always the session's worker (forced `source=self`; no backdate/foreman powers). `acknowledge:true` is the worker's "clock out anyway" past the fleet-wide open-tools block. 409 conflict bodies match the kiosk punch endpoint (the `open_checkouts` body lists tools out anywhere in the fleet, each with its building). |
